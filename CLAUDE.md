...
```bash
swift build            # compila
swift test             # testa multipart/respostas OpenAI e detecção de silêncio
...
- Env vars opcionais: `PYNKARO_MODEL`, `PYNKARO_CLAUDE_EFFORT`, `OPENAI_TRANSCRIPTION_MODEL`, `PYNKARO_FINAL_SILENCE_MS`, `PYNKARO_WAKE_WORD`, `PYNKARO_WEB_SEARCH=0`, `PYNKARO_VOICE`, `ELEVENLABS_VOICE_ID`, `ELEVENLABS_MODEL`.
...
- `OpenAITranscriptionClient.swift` — quando há chave, envia o M4A como multipart para `/v1/audio/transcriptions`, usando `OPENAI_TRANSCRIPTION_MODEL` (padrão `gpt-4o-transcribe`), `language=pt` e resposta JSON. Ausência ou falha da OpenAI aciona o fallback do Speech framework sem pedir nova gravação; temporários são apagados em todos os caminhos.
